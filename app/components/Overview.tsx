...
const Overview = () => {
  ...
  useEffect(() => {
    if (incomeTransactions && expenseTransactions && incomeCategories && expenseCategories) {
      setLoading(false);
    }
  }, [incomeTransactions, expenseTransactions, incomeCategories, expenseCategories]);
  ...
  const getTransactionsByCategory = (transactions: any[] = [], categories: any[] = []) => {
    return categories.map(category => {
      ...
      const percentage = transactions?.length 
        ? (totalAmount / transactions.reduce((sum, t) => sum + t.amount, 0)) * 100 
        : 0;
  ...
  const incomeCategoriesWithStats = getTransactionsByCategory(incomeTransactions, incomeCategories);
  const expenseCategoriesWithStats = getTransactionsByCategory(expenseTransactions, expenseCategories);
  ...
  return (
    <div className="w-full h-full flex flex-col space-y-4 mt-16">
      ...
      <StatsCards incomeTransactions={incomeTransactions} expenseTransactions={expenseTransactions} loading={loading} />
